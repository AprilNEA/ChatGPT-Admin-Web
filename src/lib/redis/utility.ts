...
export async function validateCookie(
  ...
  const cookie_exist: Cookie | null = await redis.get(
    `cookies:${email.trim()}:${key.trim()}`
  );
  if (!cookie_exist) {
    return false;
  }
...
export async function rateLimit(email: string): Promise<LimitReason> {
  const last_request =
    Number(await redis.get(`limit:rate:${email}`)) ?? (0 as number);

  // 请求速率
  if (last_request + 5000 > Date.now()) return LimitReason.TooFast;
  ...
  if (requests_number >= 25) {
    return LimitReason.TooMuch;
  } else {
    const timestamp = Date.now();
    await redis.set(`limit:rate:${email}`, timestamp);
    const limit = await redis.zadd(`limit:${email}`, {
      score: timestamp,
      member: timestamp,
    });
    return LimitReason.NoLimit;
  }
}
...
export async function newRegisterCode(email: string): Promise<number> {
  ...
  if ((await redis.set(key, randomNumber)) == "OK") return randomNumber;
...
export async function activateRegisterCode(
  email: string,
  code: string
): Promise<boolean> {
  const key = `register:code:${email.trim()}`;
  const remote = await (
    await fetch(`https://above-camel-33559.upstash.io/get/${key}`, {
      headers: {

...
export async function getAccessToken() {
  const data = await (await fetch("https://aip.baidubce.com/oauth/2.0/token?" + new URLSearchParams({
...
export async function baiduTextSecurity(text: string) {
   ...
   */
  const data = await (await fetch(`https://aip.baidubce.com/rest/2.0/solution/v1/text_censor/v2/user_defined?access_token=${await getAccessToken()}`,
    {
      method: "POST",
      headers: {

...
describe('create and validate session token', () => {
  test('should create session token', async () => {
    const token = await new AccessControlDAL(TEST_EMAIL).newSessionToken();

    expect(token).toBeTruthy();
  });

  test('should not create new session token if it is IP', async () => {
    const token = await new AccessControlDAL(
      TEST_EMAIL,
      true
    ).newSessionToken();
  ...
  test('should validate session token', async () => {
    const isValid = await new AccessControlDAL(TEST_EMAIL).validateSessionToken(
      TEST_TOKEN
    );
